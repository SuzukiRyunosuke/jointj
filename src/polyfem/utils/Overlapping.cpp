...
namespace polyfem {
    ...
    bool is_normal_outward(const State& state, int element_id, int boundary_primitive_id, Eigen::VectorXd& normal) {
        assert(state.mesh->is_simplex(element_id));
        int e = element_id;
        int b = boundary_primitive_id;
        int dim = state.mesh->dimension();
        Eigen::RowVectorXd element_barycenter(dim), boundary_barycenter(dim), inward(dim);
        element_barycenter.setZero();
        boundary_barycenter.setZero();
        ...
        //std::cout << "e_bary: " << element_barycenter << std::endl;
        //std::cout << "b_bary: " << boundary_barycenter << std::endl;
        //std::cout << "inward: " << inward << std::endl;
        //std::cout << "normal: " << normal.transpose() << std::endl;
        //std::cout << "inward*normal: " << inward*normal << std::endl;
        return inward * normal < 0;
    }
            ...
            for (auto &lb: state.total_local_boundary) {
            	...
            	normal = vals.jac_it[0] * normal; // assuming linear geometry
                    ...
                    if (!is_normal_outward(state, e, gid, normal)) {
                        normal_flipped = true;
                        normal = -1 * normal; // make normal outward for the element
                    }
                    for (int lv_id = 0; lv_id < state.mesh->dimension(); ++lv_id) {
