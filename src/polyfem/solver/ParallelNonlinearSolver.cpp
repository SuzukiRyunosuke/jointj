...
namespace cppoptlib
{
    ParallelNonlinearSolver::ParallelNonlinearSolver(
    ...
    void ParallelNonlinearSolver::set_line_search(const polyfem::json &line_search_params)
    {
    ...
    void ParallelNonlinearSolver::minimize(ProblemType &objFunc, TVector &x)
    {
        ...
        reset(objFunc); // place for children to initialize their fields

        logger().info("x.size()={}", x.size());
        Eigen::MatrixXd grads = Eigen::MatrixXd::Zero(x.rows(), objFunc.n_objectives());
...

        {
            ...
            objFunc.gradients(x, grads);
        }
        double first_grad_norm = compute_grad_norm(x, grads.rowwise().sum());
        ...
        do
        {
            ...
            const double grad_norm = compute_grad_norm(x, grads.rowwise().sum());
            ...
            if (!objFunc.compute_update_direction(x, grads, delta_x))
            {
                this->m_status = Status::Continue;
                continue;
            }

            Eigen::VectorXd grad = grads.rowwise().sum();
            assert(delta_x.cols()==1);
            assert(delta_x.rows()==grad.rows());
            //if (name() != "MMA" && grad_norm != 0 && delta_x.dot(grad) >= 0)
            if (false)
            {
                objFunc.increase_descent_strategy();
                logger().debug(
                    "[{}] direction is not a descent direction (‖Δx‖={:g}; ‖g‖={:g}; Δx⋅g={:g}≥0); reverting to {}",
                    name(), delta_x.norm(), grad.norm(), delta_x.dot(grad), objFunc.descent_strategy_name());
                this->m_status = Status::Continue;
                continue;
            }
            ...
            if (std::isnan(delta_x_norm))
            {
                objFunc.increase_descent_strategy();
                this->m_status = Status::UserDefined;
                logger().debug("[{}] Δx is nan; reverting to {}", name(), objFunc.descent_strategy_name());
                this->m_status = Status::Continue;
                continue;
            }
            ...
            this->m_current.xDelta = objFunc.descent_strategy == 2 ? NaN : (delta_x_norm / dt);
    ...
    double ParallelNonlinearSolver::line_search(const TVector &x, const TVector &delta_x, ProblemType &objFunc)
    {
        ...
        if (std::isnan(rate) && objFunc.descent_strategy < 2) // 2 is the max, grad descent
        {
            objFunc.increase_descent_strategy();
            polyfem::logger().warn(
                "[{}] Line search failed; reverting to {}", name(), objFunc.descent_strategy_name());
            this->m_status = Status::Continue; // Try the step again with gradient descent
        }
        else if (std::isnan(rate))
        {
            assert(objFunc.descent_strategy == 2); // failed on gradient descent
    ...
    void ParallelNonlinearSolver::reset(ProblemType &objFunc)
    {
        this->m_current.reset();
        objFunc.descent_strategy = objFunc.default_descent_strategy();
        m_error_code = ErrorCode::SUCCESS;
    ...
    void ParallelNonlinearSolver::reset_times()
    {
        total_time = 0;
        grad_time = 0;
        assembly_time = 0;
    ...
    void ParallelNonlinearSolver::update_solver_info(const double energy)
    {
    ...
    void ParallelNonlinearSolver::log_times()
    {
